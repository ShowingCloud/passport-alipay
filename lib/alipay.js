...
module.exports = class Alipay {
  ...
  generateSign(params) {
    const qs = toQueryString(paramsFilter(params));
    const signed = crypto.createSign('RSA-SHA256').update(qs, 'utf8').sign(fs.readFileSync(this.options.privateKey, 'utf8'), 'base64');
    return signed;
  }

  verifySign(params, signature) {
    const qs = toQueryString(paramsFilter(params));
    const verified = crypto.createVerify('RSA-SHA256').update(qs, 'utf8').verify(fs.readFileSync(this.options.alipayPublicKey, 'utf8'), signature, 'base64');
    return verified;
  }
