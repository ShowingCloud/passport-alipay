...
Alipay.prototype._generateSign = function (params) {
    var qs = toQueryString(paramsFilter(params));
    var signed = crypto.createSign('RSA-SHA256').update(new Buffer(qs, 'utf8')).sign(fs.readFileSync(this._options.private_key), 'base64');
    return signed;
};

Alipay.prototype._verifySign = function (params, signature) {
    var qs = toQueryString(paramsFilter(params));
    var verified = crypto.createVerify('RSA-SHA256').update(new Buffer(qs, 'utf8')).verify(fs.readFileSync(this._options.alipay_public_key), signature, 'base64');
    return verified;
};
